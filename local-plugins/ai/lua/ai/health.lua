...
local function check_opencode()
  ...
  -- Check template
  local template_path = Paths.legacy_template("opencode")
...
local function check_claude_code()
  ...
  -- Check template
  local template_path = Paths.legacy_template("claude_code")
  if vim.fn.filereadable(template_path) == 1 then
...
local function check_avante()
  ...
  -- Check build artifacts（avante.builder 非本插件模块，使用 pcall 检测）
