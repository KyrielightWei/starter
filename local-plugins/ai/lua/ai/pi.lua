...
local function get_repo_root(opts)
	return (opts and opts.repo_root) or Paths.config_dir()
end

local function get_config_dir(opts)
	return (opts and opts.config_dir) or Paths.config_dir()
end
...
-- 使用 Paths.join 替代本地 path_join
local path_join = Paths.join
...
local function read_jsonc(path)
	...
	vim.notify("Pi: 模板解析失败: " .. (err or path), vim.log.levels.ERROR)
	return nil
end
...
local function resolve_settings_template_path(opts)
	return template_version_path(opts)
end
...
-- 收集 packages 目录下所有 package 的文件（每个 package 是一个子目录）
local function collect_package_files(mappings, packages_root)
	local pkg_dirs = vim.fn.glob(packages_root .. "/*", false, true) or {}
	table.sort(pkg_dirs)
	for _, pkg_dir in ipairs(pkg_dirs) do
		if vim.fn.isdirectory(pkg_dir) == 1 then
			local pkg_name = vim.fn.fnamemodify(pkg_dir, ":t")
			local files = vim.fn.glob(pkg_dir .. "/**/*", false, true) or {}
			table.sort(files)
			for _, source in ipairs(files) do
				if vim.fn.filereadable(source) == 1 then
					local rel = source:sub(#pkg_dir + 2)
					table.insert(mappings, {
						source = source,
						relative = path_join("packages", pkg_name, rel),
					})
				end
			end
		end
...
function M.collect_resource_mappings(opts)
	...
	collect_package_files(mappings, path_join(repo_root, "pi/packages"))
...
function M.write_config(opts)
	...
	ensure_dir(pi_target_path(opts, "packages"))
	...
	if not settings then
		vim.notify("Pi: settings 模板加载失败，跳过同步", vim.log.levels.ERROR)
		return false
	end
	if not models then
		vim.notify("Pi: models 模板加载失败，跳过同步", vim.log.levels.ERROR)
		return false
	end
