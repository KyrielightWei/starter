...
describe("ai.state module", function()
  ...
  describe("get()", function()
    ...
    it("returns a table with provider and model keys", function()
      ...
      -- 键存在，但初始值为 nil
      assert.is_true(result.provider == nil or type(result.provider) == "string")
      assert.is_true(result.model == nil or type(result.model) == "string")
    end)
  end)
  ...
  describe("State template version", function()
    ...
    it("clears template versions on clear()", function()
      State.set_template_version("opencode", "dev")
      State.clear()
      local version = State.get_template_version("opencode")
      assert.are.equal("default", version)
    end)

    it("supports multiple tools", function()
      State.set_template_version("opencode", "secure")
      State.set_template_version("claude_code", "minimal")
      assert.are.equal("secure", State.get_template_version("opencode"))
      assert.are.equal("minimal", State.get_template_version("claude_code"))
    end)
  end)
end)
