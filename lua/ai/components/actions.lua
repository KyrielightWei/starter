...
-- Per D-36: Progress window integration for real-time progress streaming

local M = {}

-- Import ProgressWindow for real-time progress per D-36
local ProgressWindow = require("ai.components.progress_window")
...
function M.install(component_name, opts)
  ...
  -- D-36: Auto-open progress window
  local on_progress = ProgressWindow.open(component_name, "install")
  ...
  if #missing > 0 then
    -- 显示依赖缺失对话框
    on_progress("Missing dependencies detected...")
  ...
  if comp.is_installed() and not opts.force then
    on_progress("Already installed")
    ProgressWindow.handle_complete(true, "Already cached")
    return true, string.format("Component '%s' is already installed", component_name)
  end

  -- 执行安装 - pass on_progress callback per D-36
  on_progress("Starting installation...")

  -- WR-05: Wrap component call in pcall to catch exceptions
  local call_ok, call_result = pcall(function()
    return comp.install(opts, function(progress)
  ...
  local ok, msg
  if not call_ok then
    -- Component method threw an exception
    ok = false
    msg = tostring(call_result)
    ProgressWindow.handle_complete(false, msg)
    return ok, msg
  end

  ok, msg = call_result

  -- D-39: Handle complete
  ProgressWindow.handle_complete(ok, msg)
...
function M.update(component_name, opts)
  ...
  -- D-36: Auto-open progress window
  local on_progress = ProgressWindow.open(component_name, "update")

  -- 检查是否已安装
  if not comp.is_installed() then
    on_progress("Not installed")
    ProgressWindow.handle_complete(false, "Not installed - run install first")
  ...
  if version_info.status == "current" then
    ...
    ProgressWindow.handle_complete(true, "Already up to date")
  ...
  -- 执行更新 - pass on_progress callback per D-36
  on_progress("Starting update...")

  -- WR-05: Wrap component call in pcall to catch exceptions
  local call_ok, call_result = pcall(function()
    return comp.update(opts, function(progress)
      on_progress(progress)
    end)
  end)
...
function M.uninstall(component_name, opts)
  ...
  -- D-36: Auto-open progress window
  local on_progress = ProgressWindow.open(component_name, "uninstall")

  -- 检查是否已安装
  if not comp.is_installed() then
    on_progress("Not installed")
    ProgressWindow.handle_complete(true, "Not installed")
    return true, string.format("Component '%s' is not installed", component_name)
  end

  -- 确认对话框
  if not opts.force then
    -- Close progress window for dialog
    ProgressWindow.close()
    ...
    -- Reopen progress window after confirmation
    on_progress = ProgressWindow.open(component_name, "uninstall")
  end

  -- 执行卸载 - pass on_progress callback per D-36
  on_progress("Starting uninstall...")

  -- WR-05: Wrap component call in pcall to catch exceptions
  local call_ok, call_result = pcall(function()
    return comp.uninstall(opts, function(progress)
      on_progress(progress)
    end)
  end)
...
function M.show_dependency_dialog(missing, component_name)
  ...
  -- WR-08 fix: Remove auto-close, let user close manually
  -- Auto-close interrupts users reading dependency info or install hints
end
...
function M.open_config_dir(component_name)
  ...
  -- CR-01 fix: Use fnameescape for safe path handling
