...
--- WR-04: 上次文件修改时间 (cache invalidation)
---@type number|nil
...
--- 加载状态
--- WR-04: 使用 mtime-based cache invalidation
--- WR-09: 使用 sandboxed load() 防止任意代码执行
---@return table
function M.load_state()
  -- WR-04: Check file modification time for cache invalidation
  ...
  if vim.fn.filereadable(STATE_PATH) == 1 then
    ...
    if #content > 0 then
      ...
      -- WR-09: 尝试执行 Lua 代码（sandboxed）
      local ok, state = pcall(function()
        -- Use sandboxed load with empty environment to prevent access to globals
        local func = load(code, "state", "t", {})
        return func and func() or DEFAULT_STATE
      end)
      ...
      if ok and type(state) == "table" then
        ...
        -- WR-04: Update last_mtime after successful load
...
--- 清除缓存（强制重新读取）
--- WR-04: Reset last_mtime when clearing cache
...
--- 重置状态到默认值
--- WR-04: Reset last_mtime when resetting state
function M.reset()
  M._state_cache = nil
  last_mtime = nil
