...
├── theme.template.json             # Kanagawa Wave 主题
...
│   ├── permission-gate.template.ts # 拦截 rm -rf / sudo / chmod 777
│   ├── protected-paths.template.ts # 阻止写入 .env / *.pem / *.key 等
...
| Extension | 职责 |
|---|---|
| `permission-gate` | 拦 `rm -rf` / `sudo` / `chmod 777` 等高危 bash |
| `protected-paths` | 阻止 `.env` / `*.pem` / `*.key` / `id_rsa*` 的写入 |
