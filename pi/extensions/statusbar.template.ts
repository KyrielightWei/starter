...
async function requestChatGptUsage(
...
function queueChatGptUsageUpdate(ctx: ExtensionContext, force = false): void {
	void requestChatGptUsage(ctx, force).catch((error: unknown) => {
		const message = error instanceof Error ? error.message : "error";
		chatGptCache = {
...
const SEG_BG = "#E6E4D9"; // 略浅于终端背景的深色
...
function segment(
	...
	return `${bgAnsi} ${coloredText} ${reset}`;
}

const SEG_SEP = "\x1b[2m·\x1b[0m"; // dim 中点分隔符

function renderChatGptLine(
	theme: ExtensionContext["ui"]["theme"],
): string | undefined {
	if (!DEFAULT_CONFIG.showChatGptQuota) return undefined;
	if (!chatGptCache.snapshot && !chatGptCache.status) return undefined;

	const status = chatGptCache.status
		? theme.fg("dim", " (" + chatGptCache.status + ")")
		: "";
	if (!chatGptCache.snapshot)
		return theme.fg("dim", "ChatGPT: " + chatGptCache.status);
	...
	const parts = [
		fiveHour
			? theme.fg(
					chatGptColor(fiveHour),
					"5h " +
						formatPercent(fiveHour) +
						" / " +
						formatReset(fiveHour.resetAt),
				)
			: "",
		weekly
			? theme.fg(
					chatGptColor(weekly),
					"W " + formatPercent(weekly) + " / " + formatReset(weekly.resetAt),
				)
			: "",
	].filter(Boolean);

	return parts.length > 0
		? "ChatGPT: " + parts.join(theme.fg("dim", " / ")) + status
		: undefined;
}
...
function renderPrimaryLine(
	...
	const statStr = segment(theme, statColor, data.agentActive ? "run" : "idle");
	const tokInStr = segment(theme, "muted", "in:" + fmt(data.usage.input));
	const tokOutStr = segment(theme, "muted", "out:" + fmt(data.usage.output));
	const costStr = segment(theme, "text", "$" + data.usage.cost.toFixed(4));
	const cacheStr =
		data.cacheTot > 0
			? segment(
					theme,
					"success",
					"C:" + fmt(data.usage.cacheRead) + " " + data.cacheRate + "%",
				)
			: "";
...
function renderSessionLine(
	...
	const spStr = segment(
		...
		"speed:" + fmt(data.turnSpeed) + "/s",
	);
	const durStr = segment(theme, "dim", "dur:" + data.dur);
	const branchStr = segment(theme, "success", "git:" + data.branch);
	const cwdStr = segment(theme, "muted", "dir:" + data.cwd);
	const modelStr = segment(theme, "accent", data.fullModel);
	const tiStr = segment(
		theme,
	...
	return [spStr, durStr, branchStr, cwdStr, modelStr, tiStr].join(SEG_SEP);
}
...
function renderContextLine(
	...
	for (let i = 0; i < 10; i++) {
		if (i < filled) {
			...
			barChars.push(theme.fg(segCol, BAR_FULL));
		} else {
			barChars.push(theme.fg("dim", BAR_EMPTY));
		}
	}
	const barStr = barChars.join("");

	return [
		segment(theme, "dim", "Ctx"),
		segment(theme, pctCol, pctNum + "%"),
		segment(theme, pctCol, barStr),
		segment(theme, "muted", fmt(data.curTok) + "/" + fmt(data.ctxWin)),
	].join(SEG_SEP);
}
...
function renderCompactLines(
	...
	const status = data.agentActive
		? theme.fg("accent", "run")
		: theme.fg("success", "idle");
	const pctCol = contextColor(data.pct);
	const ctxStr = segment(theme, pctCol, "Ctx " + Math.round(data.pct) + "%");
	...
	const branchStr = segment(theme, "success", "git:" + data.branch);
	return [[status, ctxStr, speedStr, branchStr].join(SEG_SEP)];
}

function renderNormalLines(
	data: RenderData,
	theme: ExtensionContext["ui"]["theme"],
): string[] {
	const pctCol = contextColor(data.pct);
	...
	const tokenStr = theme.fg(
		"muted",
		"in:" + fmt(data.usage.input) + " out:" + fmt(data.usage.output),
	);
	const costStr = segment(theme, "text", "$" + data.usage.cost.toFixed(4));
	const sessionBits = [
		segment(theme, "dim", "dur:" + data.dur),
		segment(theme, "success", "git:" + data.branch),
		segment(theme, "muted", "dir:" + data.cwd),
		segment(theme, "accent", data.fullModel),
		segment(theme, "dim", "think:" + data.thinking),
	];

	return [
		[status, ctxStr, speedStr, tokenStr, costStr].join(SEG_SEP),
		sessionBits.join(SEG_SEP),
	];
}
...
function renderLines(
	...
	return lines
		.slice(0, DEFAULT_CONFIG.maxLines)
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("statusbar", {
		...
		handler: (_args: unknown, ctx: ExtensionContext) => {
			enabled = !enabled;
			if (!enabled) {
