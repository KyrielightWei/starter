...
describe("Pi sync module", function()
  ...
  it("falls back to legacy pi.template.jsonc when no versioned template exists", function()
    local settings = Pi.generate_settings({ config_dir = temp_dir("pi-config"), repo_root = project_root() })

    assert.are.equal("bailian", settings.defaultProvider)
    assert.is_table(settings.packages)
  end)
  ...
  it("writes theme file using the theme name", function()
    local pi_dir = temp_dir("pi-home")
    ...
    assert.are.equal(1, vim.fn.filereadable(pi_dir .. "/themes/flexoki-dark.json"))
  end)
  ...
  it("uses manifest hashes to back up user-modified managed resources", function()
    ...
    local prompt_path = pi_dir .. "/prompts/commit.md"
    assert.are.equal(1, vim.fn.filereadable(prompt_path))

    vim.fn.writefile({ "user modified" }, prompt_path)
    assert.is_true(Pi.write_config(opts))

    assert.are.equal(1, vim.fn.filereadable(prompt_path .. ".bak.456"))
    local manifest = read_json(pi_dir .. "/.starter-sync-manifest.json")
    assert.is_table(manifest.files["prompts/commit.md"])
  end)
